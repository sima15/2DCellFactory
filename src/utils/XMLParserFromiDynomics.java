...
public class XMLParserFromiDynomics implements Serializable {
	...
	public XMLParserFromiDynomics(Element localRoot) {
		set_localRoot(localRoot);
	}

	public XMLParserFromiDynomics(String fileName) {
	...
	public void replaceXMLFile(String path) throws IOException {
	...
	@SuppressWarnings("unchecked")
	public LinkedList<XMLParserFromiDynomics> buildSetParser(String childMarkup) {
		List<Element> childList = get_localRoot().getChildren(childMarkup);
		LinkedList<XMLParserFromiDynomics> out = new LinkedList<XMLParserFromiDynomics>();
		for (Object anElement : childList) {
			out.add(new XMLParserFromiDynomics((Element) anElement));
		}
		return out;
	}
	...
	public XMLParserFromiDynomics getChild(String childName) {
		return new XMLParserFromiDynomics(getChildElement(childName));
	}
	...
	public Element getChildSuchAttribute(String childName, String attrName, String attrValue) {
		...
		for (Object aChild : childList) {
			if (((Element) aChild).getAttributeValue(attrName).equals(attrValue)) {
	...
	public double getDblAttrOfChildSuchAttribute(String childName, String[] attrName, String[] attrValue,
			String attr2Name) {
		String out = getChildSuchAttribute(childName, attrName, attrValue).getAttributeValue(attr2Name);
		return (double) Double.parseDouble(out);
	}

	public String getStrAttrOfChildSuchAttribute(String childName, String attrName, String attrValue,
			String attr2Name) {
		String out = getChildSuchAttribute(childName, attrName, attrValue).getAttributeValue(attr2Name);
		return out;
	}
	...
	public double[] getChildListAttrDbl(String childName, String attrName) {
		...
		double out[] = new double[childList.size()];
		while (iter.hasNext()) {
			out[iter.nextIndex()] = Double.parseDouble(((Element) iter.next()).getAttributeValue(attrName));
		}
		return out;
	}
